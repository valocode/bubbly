...
type BasicHCL struct {
	Locals  core.Locals       `hcl:"local,block"`
	Modules core.ModuleBlocks `hcl:"module,block"`
	Inputs  core.Inputs       `hcl:"input,block"`
	Outputs core.Outputs      `hcl:"output,block"`

	BasicBlocks []struct {
		FirstLabel     string `hcl:",label"`
		SecondLabel    string `hcl:",label"`
		Number         int    `hcl:"number,attr"`
		String         string `hcl:"string,attr"`
		OptionalString string `hcl:"optstring,optional"`
	} `hcl:"basicblock,block"`
	OptionalBlock *struct {
		Label      string   `hcl:",label"`
		StringList []string `hcl:"strlist,attr"`
	} `hcl:"optblock,block"`
	ListBlock []struct {
		FirstLabel  string   `hcl:",label"`
		SecondLabel string   `hcl:",label"`
...
basicblock "first_label" "second_label" {
	number = local.answer_to_life.value
	string = local.weather_today.value
	// optstring is optional :)
}

// optblock {} is optional
`
...
optblock "testing" {
	...
	t.Run("Basic HCL example with string list", func(t *testing.T) {
		b := BasicHCL{}
		processHCL(t, stringListHCL, &b)
		assert.Equal(t, b.OptionalBlock.Label, "testing")
		assert.Equal(t, b.OptionalBlock.StringList[0], "tic")
		assert.Equal(t, b.OptionalBlock.StringList[1], "tac")
	})
}
