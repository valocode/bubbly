...
// 	for _, r := range repos {
// 		ok, err := client.Repo.Query().Where(repository.Name(r)).Exist(h.ctx)
// 		if err != nil {
// 			return nil, HandleEntError(err, "checking repository exist")
// 		}
// 		if !ok {
// 			return nil, NewNotFoundError(nil, "repository %s does not exist", r)
// 		}
// 	}
// 	repoIDs, err := client.Repo.Query().Where(
// 		repository.NameIn(repos...),
// 	).IDs(h.ctx)
// 	if err != nil {
// 		return nil, HandleEntError(err, "getting repository IDs")
// 	}
// 	return repoIDs, nil
// }
