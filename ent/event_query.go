...
func (eq *EventQuery) QueryRelease() *ReleaseQuery {
	query := &ReleaseQuery{config: eq.config}
...
func (eq *EventQuery) QueryProject() *ProjectQuery {
	query := &ProjectQuery{config: eq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := eq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (eq *EventQuery) sqlAll(ctx context.Context) ([]*Event, error) {
	...
	if query := eq.withRelease; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Event)
		for i := range nodes {
			if nodes[i].event_release == nil {
				continue
			}
			fk := *nodes[i].event_release
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "event_release" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := eq.withProject; query != nil {
		...
		nodeids := make(map[int][]*Event)
		for i := range nodes {
			if nodes[i].event_project == nil {
				continue
			}
			fk := *nodes[i].event_project
			if _, ok := nodeids[fk]; !ok {
				ids = append(ids, fk)
			}
			nodeids[fk] = append(nodeids[fk], nodes[i])
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "event_project" returned %v`, n.ID)
			}
			for i := range nodes {
